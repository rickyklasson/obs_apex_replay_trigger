        ...
        try:
            ...
            while True:
                ...
                if event is not None:
                    logging.info(f'Game event: {event} detected at {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
                ...
                # Sleep.
                end_time = time.time()
                logging.info(f'Ran in: {end_time - start_time:.3f}s', end='\r', flush=True)
    ...
    def monitor_games(self, triggers: list[str]):
        ...
        try:
            ...
            while True:
                ...
                # Sleep.
                end_time = time.time()
                logging.debug(f'({state}) Ran in: {end_time - start_time:.3f}s', end='\r', flush=True)
                time.sleep(max(0, PERIOD - (end_time - start_time)))
        except KeyboardInterrupt:
            pass
        finally:
